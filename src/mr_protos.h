...
TYPEDEF_ENUM (mr_ptrdes_flags_t, ATTRIBUTES (__attribute__ ((packed)), "ponter descriptor flag bitfield values"),
	      ...
	      (MR_IS_REFERENCED, = 1 << 1, "pointer is already serialized somewhere else"),
	      (MR_IS_CONTENT_REFERENCE, = 1 << 2, "pointers on string content may reffer only on string pointer"),
	      (MR_IS_OPAQUE_DATA, = 1 << 3, "opaque serialization of resizeable pointer to XDR"),
	      (MR_IS_UNNAMED, = 1 << 4, "for CINIT serialization all fields are named, but anonymous union and arrays elements don't need name"),
	      (MR_IS_TYPED, = 1 << 5, "for CINIT deserialization pointer nodes has type"),
	      )
