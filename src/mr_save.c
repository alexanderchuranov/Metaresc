...
mr_union_discriminator_by_type (mr_td_t * tdp, mr_fd_t * parent_fdp, void * discriminator, mr_ic_t * ud_overrides)
{
  ...
  for (;;)
	...
	case MR_TYPE_BOOL:
	  {
	    value._unsigned = !!*(bool*)discriminator;
	    mr_fd_t * fdp = mr_ud_override_value (ud_overrides, value);
	    if (fdp)
	      return (fdp);
	    return (mr_union_discriminator_by_idx (tdp, value._unsigned));
	  }
...
#define CASE_RETURN_BY_TYPE_(TYPE, MR_TYPE)				\
	  case MR_TYPE:							\
	    {								\
	      value._unsigned = *(TYPE*)discriminator;			\
	      mr_fd_t * fdp = mr_ud_override_value (ud_overrides, value); \
	      if (fdp)							\
		return (fdp);						\
	      return (mr_union_discriminator_by_idx (tdp, value._unsigned)); \
	...
	case MR_TYPE_BITFIELD:
	  {
	    mr_uintmax_t bf_value = 0;
	    mr_ptrdes_t ptrdes = { .data.ptr = discriminator, .fdp = parent_fdp, };
	    mr_td_t * enum_tdp = parent_fdp->stype.tdp;

	    mr_save_bitfield_value (&ptrdes, &bf_value); /* get value of the bitfield */

	    value._unsigned = bf_value;
	    mr_fd_t * fdp = mr_ud_override_value (ud_overrides, value);
	    if (fdp)
	      return (fdp);
	  
	    if (MR_TYPE_ENUM == parent_fdp->stype.mr_type_aux)
	      {
	    ...
	    if (MR_TYPE_BOOL == parent_fdp->stype.mr_type_aux)
	      value._unsigned = !!value._unsigned;
	...
	case MR_TYPE_ENUM:
	  {
	    ...
	    value = mr_get_enum_value (enum_tdp, discriminator);
	    mr_fd_t * fdp = mr_ud_override_value (ud_overrides, value);
	    if (fdp)
	      return (fdp);
...
mr_ud_find (mr_ptrdes_t * ptrdes, mr_ptr_t key, mr_save_data_t * mr_save_data)
{
  switch (ptrdes->value_type)
    ...
    case MR_VT_UD_PTR:
      {
	mr_ptr_t * find = mr_ic_find (ptrdes->union_discriminator, key);
	return (find ? find->uintptr : -1);
      }

    case MR_VT_UD_SET:
      {
	mr_ud_set_t * uds = &ptrdes->ud_set;
	int i;
...
mr_ud_add (mr_ptrdes_t * ptrdes, mr_ptr_t key, mr_save_data_t * mr_save_data)
{
  switch (ptrdes->value_type)
    ...
    case MR_VT_UD_PTR:
      {
	mr_ptr_t * add = mr_ic_add (ptrdes->union_discriminator, key);
	...
	if (uds->count == sizeof (uds->idx) / sizeof (uds->idx[0]))
	  {
	    ...
	    ptrdes->union_discriminator = dst_ic;
	    ptrdes->value_type = MR_VT_UD_PTR;
...
static mr_status_t
mr_ud_foreach (mr_ptrdes_t * ptrdes, mr_visit_fn_t visit_fn, void * context)
{
  switch (ptrdes->value_type)
    {
    case MR_VT_UD_PTR:
      return (mr_ic_foreach (ptrdes->union_discriminator, visit_fn, context));
...
{
  if (ptrdes->value_type == MR_VT_UD_PTR)
    {
      mr_ic_free (ptrdes->union_discriminator);
      MR_FREE (ptrdes->union_discriminator);
      ptrdes->union_discriminator = NULL;
    }
  ptrdes->value_type = MR_VT_VOID;
}
...
mr_union_discriminator (mr_save_data_t * mr_save_data, mr_idx_t node, mr_fd_t * union_fdp)
{
  ...
  if ((ud_idx >> (__CHAR_BIT__ * sizeof (MR_RA_UD_IDX_TYPE))) != 0)
    {
  ...
  for (parent = node; parent != MR_NULL_IDX; parent = mr_save_data->ptrs.ra[parent].parent)
    {
      ...
      ud_find = mr_ud_find (parent_ptrdes, ud_idx, mr_save_data);
...
mr_save_pointer_content (mr_idx_t idx, mr_save_data_t * mr_save_data)
{
  ...
  fd_.stype.mr_type = ra[idx].mr_type_aux;
  fd_.stype.tdp = ra[idx].fdp ? ra[idx].fdp->stype.tdp : NULL;
  fd_.stype.mr_type_aux = ra[idx].fdp ? ra[idx].fdp->mr_type_base : MR_TYPE_VOID;
  ...
  mr_idx_t i = 0;
  mr_idx_t first_child = ra[idx].first_child;
  if (first_child != MR_NULL_IDX)
    {
      if (ra[first_child].mr_type == MR_TYPE_POINTER)
	mr_save_data->ptrs.last_child = MR_NULL_IDX;
      else
	{
	  i = ra[first_child].MR_SIZE / fd_.stype.size;
	  mr_save_data->ptrs.last_child =
	    mr_get_child_by_addr (mr_save_data, idx, ra[first_child].data.uintptr + (i - 1) * fd_.stype.size);
	}

      if (mr_save_data->ptrs.last_child == MR_NULL_IDX)
	i = mr_find_last_child (mr_save_data, idx, fd_.stype.size);
    }

  for (;;)
    {
      mr_idx_t count = mr_save_data->ptrs.ra[idx].MR_SIZE / fd_.stype.size;
      if (i >= count)
	break;
  ...
  first_child = mr_save_data->ptrs.ra[idx].first_child;
  if (!(mr_save_data->ptrs.ra[idx].flags & MR_IS_REFERENCE) && (mr_save_data->ptrs.ra[first_child].mr_type != MR_TYPE_POINTER))
      mr_save_data->ptrs.ra[first_child].MR_SIZE = mr_save_data->ptrs.ra[idx].MR_SIZE;
  mr_save_data->ptrs.ra[idx].flags &= ~MR_IS_POSTPONED;
...
resolve_pointer (mr_save_data_t * mr_save_data, mr_idx_t ref_idx, bool * resolved)
{
  ...
  *resolved = false;
  ...
  mr_size_t ref_mr_size = ra[ref_idx].MR_SIZE;
  if (ref_parent != MR_NULL_IDX)
    if ((ra[ref_parent].mr_type == MR_TYPE_POINTER) || (ra[ref_parent].mr_type == MR_TYPE_ARRAY))
      {
	mr_idx_t ref_first_child = ra[ref_parent].first_child;
	if (ref_first_child != MR_NULL_IDX)
	  ref_mr_size = ra[ref_parent].MR_SIZE - (ra[ref_idx].data.ptr - ra[ref_first_child].data.ptr);
      }
  ...
  if (MR_NULL_IDX == ra[parent].first_child) /* this is the first element in resizable array */
      ...
      if (MR_TYPE_POINTER == ra[ref_parent].mr_type)
	{
	  ...
	  */
	  mr_idx_t first_child = ra[ref_parent].first_child;
	  if ((first_child != MR_NULL_IDX) && !(ra[ref_parent].flags & MR_IS_POSTPONED))
	    {
	      /*
		Here we detect that pointer content needs to be resized for the first time. In that case we store pointer node
		into stack for postponed loading. No need to add the same node to the stack in case if we will detect consecutive
		cases of resize for the same pointer.
	      */
	      mr_idx_t * idx = mr_rarray_allocate_element ((void*)&mr_save_data->postponed,
							   &mr_save_data->postponed_size, &mr_save_data->postponed_alloc_size,
							   sizeof (mr_save_data->postponed[0]));
	      if (NULL == idx)
		return (0);
	      *idx = ref_parent;
	      ra[ref_parent].flags |= MR_IS_POSTPONED;
	    }
	  ra[ref_parent].MR_SIZE += ra[idx].MR_SIZE - ref_mr_size;
	  ra[parent].first_child = ref_idx;
	  ra[parent].flags |= MR_IS_REFERENCE;
	  ra[ref_idx].flags |= MR_IS_REFERENCED;
	  *resolved = true;
  ...
  else
    {
	  ...
	  !ref_is_parent (ra, parent, ref_parent))
	{
	  /*
	  ...
	  if (ref_mr_size > ra[idx].MR_SIZE)
	    {
	      ...
	      if (ra[first_child].mr_type != MR_TYPE_POINTER)
		ra[first_child].MR_SIZE = ra[parent].MR_SIZE;
	    }
...
resolve_matched (mr_save_data_t * mr_save_data, mr_idx_t ref_idx, bool * resolved)
{
  ...
  for ( ; ref_idx != MR_NULL_IDX; ref_idx = ra[ref_idx].idx)
    {
      mr_idx_t ref_parent = ra[ref_idx].parent;
...
mr_save_inner (void * data, mr_fd_t * fdp, mr_idx_t count, mr_save_data_t * mr_save_data, mr_idx_t parent)
{
  ...
  if (search_result->uintptr != idx)
	...
	return (nodes_matched);
      ...
      mr_save_data->ptrs.ra[idx].idx = search_result->uintptr;
...
{
  ptrs->vt_uintptr = false;
  mr_ptrs_dfs (ptrs, mr_remove_empty_node, NULL);
...
{
  mr_ra_ptrdes_t * ptrs = &mr_save_data->ptrs;
  mr_idx_t idx, count = mr_save_data->ptrs.size / sizeof (mr_save_data->ptrs.ra[0]);
...
mr_save_pointer (mr_save_data_t * mr_save_data)
{
  ...
  if (NULL == *data)
      ...
      if (src.data.ptr != NULL)
	{
	  ...
	  mr_assign_int (&dst, &src);
	  if (src.vt_string[0] == 'c') /* if pointer discriminated by count we need to scale MR_SIZE by element_size */
	    ptrdes->MR_SIZE *= element_size;
	}
...
#define MR_IC_METHOD MR_IC_HASH
  ...
  if (nodes_added > 0)
	...
	if ((MR_TYPE_POINTER == mr_save_data.ptrs.ra[i].mr_type) &&
	    ...
	    !(mr_save_data.ptrs.ra[i].flags & (MR_IS_NULL | MR_IS_OPAQUE_DATA)))
	  {
	    ...
	    if (nodes_added == 0)
	      break;
	    while (mr_save_data.postponed_size > 0)
	      {
		mr_save_data.postponed_size -= sizeof (mr_save_data.postponed[0]);
		mr_idx_t idx = mr_save_data.postponed_size / sizeof (mr_save_data.postponed[0]);
		nodes_added = mr_save_pointer_content (mr_save_data.postponed[idx], &mr_save_data);
		if (nodes_added == 0)
		  break;
	      }
	    if (nodes_added == 0)
  ...
  if (mr_save_data.postponed != NULL)
    MR_FREE (mr_save_data.postponed);
